...
import { updateStudentProgress, getIdeasByGroupId } from "../api/studentPanelApi";
...
const ProgressTracking = ({
  ...
  const handleSaveTeamClick = async () => {
    ...
    // Check if the project name is an accepted project for this group
    if (groupId) {
      try {
        ...
        if (ideasResult.success && ideasResult.ideas) {
          ...
          if (matchingIdea) {
            if (matchingIdea.status === 'Rejected') {
              alert("Project name must be an accepted project. Rejected projects cannot be saved.");
              return;
            }
            // If status is Accepted or any other status (not Rejected), allow save
          }
        }
      } catch (error) {
    ...
    onSaveProject(trimmedProjectName);
